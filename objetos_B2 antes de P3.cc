...
for (j=0;j<num;j++)
  ...
  //cout <<"Numero de vertices de la figura: " <<vertices.size() <<endl;

  //Si numVer0 es mayor o igual que 2 significa que el perfil tiene dos puntos sobre el eje y
  if(numVer0>=2)
    num_aux-=2; //Le restamos 2 para no tener en cuenta los vertices de las tapas ya que si no seria mas dificil hacer los calculos

// tratamiento de las caras 
caras.resize((num*2*(num_aux-1))+2*(num));

//cout <<"Numero de caras: " <<caras.size() <<endl;
//caras[0]._0=0;  caras[0]._1=num_aux+1;  caras[0]._2=num_aux;
//caras[1]._0=0;  caras[1]._1=1;  caras[0]._2=num_aux+1

int c=0, num_ver=vertices.size()-2; //Suponemos que el perfil no tiene puntos sobre el eje y
...
/*
//Version semifinal (Le falta el extra)
void _rotacion::parametros(vector<_vertex3f> perfil, int num)
{
  //int i,j;
  _vertex3f vertice_aux;
...
//vertices.resize((num_aux-2)*num+2);
for (j=0;j<(num);j++)
  {for (i=0;i<num_aux;i++)
     {
      if(perfil[i].x!=0.0){
